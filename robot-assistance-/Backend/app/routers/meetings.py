...
from app.schemas.poc import POCMeeting
...
def _meeting_poc_payload(document: dict) -> dict:
    data = _meeting_defaults(document)
    return {
        "id": data["id"],
        "title": data["title"],
...
async def _sync_meeting_to_google(db: Any, meeting_doc: dict) -> dict:
...
async def _sync_relative_reminders(db: Any, meeting_doc: dict) -> None:
...
async def _get_meeting_or_404(db: Any, meeting_id: int) -> dict:
...
@router.get("/meetings", response_model=list[POCMeeting])
async def get_all_meetings(db: Any = Depends(get_db)):
    docs = await db["meetings"].find().sort("start_time", ASCENDING).to_list(length=5000)
    return [POCMeeting.model_validate(_meeting_poc_payload(doc)) for doc in docs]


@router.get("/meetings/today", response_model=list[POCMeeting])
async def get_today_meetings(db: Any = Depends(get_db)):
...
@router.get("/meetings/{meeting_id}", response_model=POCMeeting)
async def get_meeting(meeting_id: int, db: Any = Depends(get_db)):
    meeting = await _get_meeting_or_404(db, meeting_id)
    return POCMeeting.model_validate(_meeting_poc_payload(meeting))
...
@router.post("/meetings", response_model=POCMeeting, status_code=status.HTTP_201_CREATED)
async def create_meeting(meeting: MeetingCreate, db: Any = Depends(get_db)):
    ...
    try:
        await log_activity(
            ...
            entity_type="meeting",
            entity_id=meeting_doc["id"],
            performed_by=str(meeting_doc.get("user_id") or "robot"),
            details={"title": meeting_doc["title"]},
        )
    except Exception:  # noqa: BLE001
        pass
    return POCMeeting.model_validate(_meeting_poc_payload(meeting_doc))


@router.put("/meetings/{meeting_id}", response_model=POCMeeting)
async def update_meeting(meeting_id: int, update: MeetingUpdate, db: Any = Depends(get_db)):
    ...
    try:
        await log_activity(
            ...
            performed_by=str(updated_doc.get("user_id") or "robot"),
            details={"title": updated_doc.get("title")},
        )
    except Exception:  # noqa: BLE001
        pass
    return POCMeeting.model_validate(_meeting_poc_payload(updated_doc))


@router.patch("/meetings/{meeting_id}/availability", response_model=POCMeeting)
...
@router.patch("/meetings/{meeting_id}/alerts", response_model=POCMeeting)
...
async def delete_meeting(meeting_id: int, db: Any = Depends(get_db)):
    meeting_doc = await _get_meeting_or_404(db, meeting_id)
    ...
    try:
        await log_activity(
            ...
            entity_id=meeting_id,
            performed_by=str(meeting_doc.get("user_id") or "robot"),
            details={"title": meeting_doc.get("title")},
        )
    except Exception:  # noqa: BLE001
        pass
